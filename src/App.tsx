...
function App() {
  ...
  function handleAddIgnore() {
    ...
    saveSettings({ ...settings, ignored_paths: [...settings.ignored_paths, newIgnorePath] });
    setNewIgnorePath("");
  }

  function handleRemoveIgnore(path: string) {
    saveSettings({ ...settings, ignored_paths: settings.ignored_paths.filter(p => p !== path) });
  }

  async function handlePickDirectory() {
    const selected = await open({ directory: true, multiple: false, title: "Select Directory to Index" });
    if (selected && typeof selected === "string") {
      setIndexing(true); setProgress(0); setMessage(`Indexing ${selected}...`);
  ...
  async function handleSearch() {
    if (!query) return;
    setSearching(true); setActiveIndex(-1); setChatResponse("");
    try {
  ...
  async function handleChat() {
    ...
    setIsChatting(true); setChatResponse("Thinking...");
    try {
      const res = await invoke<string>("ask_question", { query: chatQuery, context: results.slice(0, 5).map(r => r.snippet) });
      setChatResponse(res);
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLInputElement>) {
    if (e.key === "Enter") {
      if (activeIndex >= 0 && results[activeIndex]) invoke("open_path", { path: results[activeIndex].path });
      else handleSearch();
      ...
      <div className="search-section">
        ...
        {showFilters && !showSettings && (
          <div className="advanced-filters">
            <label><input type="checkbox" checked={isRegex} onChange={(e) => setIsRegex(e.target.checked)} /> Regex Mode</label>
        ...
        <div className="expanded-content">
          ...
          {showSettings ? (
            <div className="settings-panel">
              ...
              <div className="setting-group">
                <label>Theme</label>
                <select value={settings.theme} onChange={(e) => saveSettings({ ...settings, theme: e.target.value })} className="theme-select">
              ...
              <div className="setting-group">
                ...
                <div className="settings-input-group">
                  <input value={newIgnorePath} onChange={(e) => setNewIgnorePath(e.target.value)} placeholder="Add pattern..." onKeyDown={(e) => e.key === "Enter" && handleAddIgnore()} />
                  <button onClick={handleAddIgnore}>Add</button>
                </div>
                <ul className="ignore-list">
                  {settings.ignored_paths.map((p, i) => <li key={i}><span>{p}</span><button className="remove-btn" onClick={() => handleRemoveIgnore(p)}>Remove</button></li>)}
                </ul>
              </div>
            </div>
          ) : (
            <div className="main-content-area">
              {message && (
                <div className="progress-container">
                  ...
                  {indexing && <div className="progress-bar"><div className="progress-fill" style={{ width: `${progress}%` }}></div></div>}
                </div>
              )}
              ...
              {results.length > 0 && (
                <div className="chat-panel">
                  <h3>Chat with files</h3>
                  <div className="chat-input-group">
                    <input value={chatQuery} onChange={(e) => setChatQuery(e.target.value)} placeholder="Ask a question..." onKeyDown={(e) => e.key === "Enter" && handleChat()} />
                    <button onClick={handleChat} disabled={isChatting || !chatQuery}>{isChatting ? "..." : "Ask"}</button>
                  </div>
          ...
          <div className="footer-controls">
            <button onClick={handlePickDirectory} disabled={indexing}>+ Index Folder</button>
